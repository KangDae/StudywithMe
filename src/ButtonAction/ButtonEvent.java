...
public class ButtonEvent extends ButtonAccemble implements ActionListener, MouseListener{
	...
	public void event() {
		...
		// ================== 시작 프레임 부분 ======================
		start_btn_loginBtn.addActionListener(this);
		start_btn_SingUp.addActionListener(this);
		btnExit.addActionListener(this);
		// ================== 회원가입 프레임 부분 ======================
		signup_btn_IDCheck.addActionListener(this);
		signup_btn_EmailSend.addActionListener(this);
		signup_btn_Auth.addActionListener(this);
		signup_btn_Cancle.addActionListener(this);
		signup_btn_Check.addActionListener(this);
		// ================== 로그인 프레임 부분 ======================
		StudyWithMe.addMouseListener(this);
		login_btn_Login.addActionListener(this);
		login_btn_SearchID.addActionListener(this);
		login_btn_SearchPW.addActionListener(this);
		login_btn_Exit.addActionListener(this);
		// ================== 아이디 찾기 부분 ======================
		searchi_btn_Send.addActionListener(this);
		searchi_btn_Auth.addActionListener(this);
		searchi_btn_Cancle.addActionListener(this);
		searchi_btn_Exit.addActionListener(this);
		searchi_btn_Confilrm.addActionListener(this);
		// ================== 비밀번호 찾기 부분 ======================	
		searchp_btn_Send.addActionListener(this);
		searchp_btn_Confirm.addActionListener(this);
		searchp_btn_Auth.addActionListener(this);
		searchp_btn_pwCancle.addActionListener(this);
		searchp_btn_pwExit.addActionListener(this);
		searchp_btn_Reset.addActionListener(this);
		// ================== 재설정 부분 ======================	
		reset_btn_pwCheck.addActionListener(this);
		reset_btn_Cancle.addActionListener(this);
		reset_btn_Reset.addActionListener(this);
	...
	public void actionPerformed(ActionEvent e) {
		...
		// ==> 확인버튼 구현 <==
			...
			if (name.length() == 0 || id.length() == 0 || password.length() == 0 
					|| email1.length() == 0 || emailCheck.length() == 0 ) { 																					// emailCheck.length()==0
		...
		// ==> 보내기 버튼 구현 <==
			...
			if (frameSearchID.textField_Email.getText().length() == 0) {
				...
				JOptionPane.showMessageDialog(btn_Confirm, "인증번호가 전송되었습니다.");
				String emailStr = frameSearchID.textField_Email.getText() + '@' + (String) frameSearchID.comboBox_Email.getSelectedItem();
				scretNumber = String.valueOf(SendMail_update.SendMail(emailStr));
		...
		// ==> 아이디 확인 버튼 <==
			...
			if (name.length() == 0 || year.length() == 0 || moonth.length() == 0 || day.length() == 0
				...
				String line = "";
				line += name + "%" + year + moonth + day + "%" + email + '@' + frameSearchID.comboBox_Email.getSelectedItem();
		...
		// ==> 전송 버튼 기능 구현 <==
			...
			if (frameSearchPW.textField_Email.getText().length() == 0) {
				...
				JOptionPane.showMessageDialog(btn_Confirm, "인증번호가 전송되었습니다.");
				String emailStr = frameSearchPW.textField_Email.getText() + '@' + (String) frameSearchPW.comboBox_Email.getSelectedItem();
				scretNumber = String.valueOf(SendMail_update.SendMail(emailStr));
				System.out.println(scretNumber);
			}
		}
		...
		// ==> 재설정버튼 기능 구현 <==
			...
			if(name.length()==0) {
				...
				frameSearchPW.textField_EmailCheck.setText("");
