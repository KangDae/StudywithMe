...
//						+ userContent[1] + "')";
						...
						if (count == 0) // ID,PW 틀리면
						{
							...
							for (int i = 0; i < noticeList.size(); i++) { //0 prynum 1 title 2 writer 3 date 4 content 5 -
								noticeMessage += (noticeList.get(i).getPryNumber() + "%"
										+ noticeList.get(i).getTitle() + "%"
										+ noticeList.get(i).getName() + "%"
										+ noticeList.get(i).getDate() + "%" + noticeList.get(i).getContent()
										+ "*");
							}
					...
					for (int i = 0; i < roomtotalList.get(index).roomInUserList.size(); i++) {
						roomtotalList.get(index).roomInUserList.get(i).pw.println(Protocol.ENTERROOM_USERLISTSEND + "|"
								+ LinesTest + "|" + roomMember + "|" + user.getIdName() + "님이 입장하셨습니다.|" + fileList);
					...
					int roomIndex = 0;
					boolean con = false;// 남아있는방에 최소 2명이상일때
				...
				} else if (line[0].compareTo(Protocol.CHATTINGFILEDOWNLOAD_SYN) == 0) // 파일 다운로드 보냄
				{
					...
					String selectsql = "select * from noticeboard where Title = '" + line[1] + "' and  Name= '" + this.user.getIdName()
					+ "' and  date= '" + now.toString() + "' and  content= '" + line[2] + "'";
				 ...
				 String lines = "";
				  ...
				  for(int i =0; i< noticeList.size(); i++) {
					  if(noticeList.get(i).getPryNumber() == Integer.parseInt(prynum)) {
						  ...
						  lines = prynum +"|"+ noticeList.get(i).getTitle() +"|"+ noticeList.get(i).getName() +"|"+ noticeList.get(i).getContent();
